...
def create_posts(post: schemas.PostBase, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    #                                                                                          post.content, post.published)) # instead of f-string to prevent sql injection
    # new_post = cursor.fetchone()
    # conn.commit()

    # new_post = models.Post(title=post.title, content=post.content, published=post.published)
    new_post = models.Post(owner_id=current_user.id, **post.dict()) # unpack dictionary with **
    db.add(new_post)
    db.commit()
    db.refresh(new_post)
    return new_post
...
def get_post(id: int, response: Response, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    post = db.query(models.Post).filter(models.Post.id == id).first() # returns first one
